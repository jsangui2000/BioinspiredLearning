...
static void mainLoop(void)
{
	...
	if (comandoNum==-1)
	{
		int received = receiveMessage(comando);
		if(received>0)
		{
			puts(comando);
			if(strcmp(comando,"comando0")==0){
				comandoNum =0;
				puts("0");
			}else if(strcmp(comando,"comando1")==0){
				comandoNum =1;
				puts("1");
	...
	if(comandoNum!=-1 && object[comandoNum].visible!=0)
	{

		sendMessage(object[comandoNum].trans[0][3],
				    object[comandoNum].trans[1][3]);
		comandoNum = -1;
	}
